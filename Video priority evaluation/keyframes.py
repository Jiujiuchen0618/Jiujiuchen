...
class KeyFrameGetter:
    ...
    Get the key frame
    '''
    def __init__(self, video_path, img_path, window):
        '''
    ...
    def plot_diff_time(self):
        ...
        #取文件名
        filename = self.video_path.split('.')[0]
        filename = filename.split('/')[-1]
        np.savetxt(filename+'.txt', self.diff)
        plt.plot(np.array(self.idx)-1, [self.diff[i] for i in (np.array(self.idx)-1)], 'or')
...
if __name__ == '__main__':
    kfg = KeyFrameGetter('video/set10v004.avi', 'img', 25)
